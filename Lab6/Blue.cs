...
namespace Lab6
{
    public class Blue
    {
        ...
        {
            int s = 0, mx = int.MinValue, mn = int.MaxValue;
            for (int i = 0; i < m.GetLength(0); i++)
                for (int j = 0; j < m.GetLength(1); j++)
                {
                    int v = m[i, j];
        ...
        public void RemoveCol(ref int[,] m, int c)
        {
        ...
        {
            int r, c;
            return FindMax(m, out r, out c);
        }

        public int FindMin(int[,] m)
        {
            int r, c;
            return FindMin(m, out r, out c);
        }
        ...
        public double SumB(double x)
        {
            ...
            for (int i = 1; ; i++)
            {
                double t = Math.Pow(-1, i) * Math.Cos(i * x) / (i * i);
                s += t;
        ...
        // ===== TASK 10 =====
        public bool Task10(int[][] a, Predicate<int[][]> f)
        {
            return f(a);
        }
